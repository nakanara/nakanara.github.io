...
Oracle 19c는 다중 모델를 목적으로 나온 데이터베이스입니다. 
최근 오라클 19c로 사용이 증가하여 설치를 진행하였으며, 설치하는 과정을 작성하였습니다.

/app아래 설치하고자 경로를 일부 변경하였습니다.
...
yum을 통한 설치전 사전 설정 작업을 진행합니다. 
자동 설정이 아닌 수동 설정을 할 경우 "수동 설정"으로 진행합니다.
...
습관처럼 yum을 바로 설치하지 않고, 다운 받은 후 설치를 진행하였습니다.
...
/app경로 아래 DB 엔진이 설치될 /oracle과 데이터가 저장될 /oradata 를 생성하였습니다.
...
오라클 홈페이지에서 Oracle 19c 압축파일을 ORACLE_HOME에 압푹을 풉니다.
...
/etc/oratab 파일의 각 인스턴스별 재시작 플래그가 N을 Y로 설정하여 자동 재시작 되도록 수정합니다.
